...
**Analysis:** Incremental update (HEAD~5..HEAD — 38 files + 10 uncommitted)
...
| **Architecture** | Dual-structure (dev ↔ npm-templates mirror) |
| **Language** | JavaScript (Node.js ≥16) |
| **Author** | Shamavision / Wellme Studio (Ukraine 🇺🇦) |
| **Complexity** | 🟢 Low-Medium (4/10) ← reduced after Tasks 4-6 |
...
## 🔴 CRITICAL: Dual-Structure Architecture

**Every change MUST be made in TWO places simultaneously:**

```
Dev files                      →   npm-templates/ (distributable)
───────────────────────────────────────────────────────────────
.claude/CLAUDE.md              →   npm-templates/.claude/CLAUDE.md
.cursorrules                   →   npm-templates/.cursorrules
AGENTS.md                      →   npm-templates/AGENTS.md
.ai/AI-ENFORCEMENT.md          →   npm-templates/.ai/AI-ENFORCEMENT.md
.ai/rules/core.md              →   npm-templates/.ai/rules/core.md
.ai/rules/product.md           →   npm-templates/.ai/rules/product.md
.ai/contexts/*                 →   npm-templates/.ai/contexts/*
.ai/presets.json               →   npm-templates/.ai/presets.json
scripts/*                      →   npm-templates/scripts/*
...
|------|---------|-------------|
| `ROADMAP.md` | Source of truth — Tasks + status | Medium (gitignored) |
| `PROJECT_CONTEXT_MAP.md` | This file — current architecture snapshot | Light (~2k) |
...
| `.ai/presets.json` | Tool/plan presets v1.0 — session_limit + billing type | Light |
...
| `npm-templates/` | Mirror directory — must stay in sync with dev | Heavy |
| `.claude/commands/` | Skills: /ctx, /sculptor, /arbiter | Light (×3) |

**⛔ NEVER read whole file:** `.ai/token-limits.json` (still exists but 840+ lines)
...
```
/ctx (Реальність) → /sculptor (Чистота) → /arbiter (Порядок + Безпека)
```
...
| Entry Point | Purpose |
|-------------|---------|
| `bin/cli.js` | NPX entry: `npx @shamavision/ai-workflow-rules` → interactive wizard |
| `scripts/install.sh` | Bash install alternative (⚠️ wizard questions outdated — see Wizard Redesign Backlog) |
...
| Task | Status | Result |
|------|--------|--------|
| Task 1 | ✅ DONE | /ctx + /sculptor + auto-monitoring pipeline |
| Task 2 | ✅ DONE | Remove Windsurf/Continue.dev → rule-files 10→6 |
| Task 3 | ✅ DONE | Pre-commit hook: 5-15 min → <2 sec, false positives fixed |
| Task 4 | ✅ DONE | Reduce presets 4→2 (minimal + ukraine-full) |
| Task 5 | ✅ DONE | inquirer@8 → @clack/prompts (-44 packages) |
| Task 6 | ✅ DONE | presets.json v1.0 — removed MODEL_1/2/3 + fake limits |
| Task 7 | ⏳ UNCOMMITTED | Session memory anchor — post-push.sh + anchor in CLAUDE.md/AGENTS.md |
| Task 8 | ✅ DONE | /arbiter skill — triangle complete, all 3 in installer |

**Deferred:**
- Wizard Redesign Backlog — after new analysis with /arbiter
...
```
presets.json        → static: session_limit (public docs), billing type
token-limits.json   → user config: tool + session_limit (generated at install)
session-log.json    → dynamic: sessions today, estimated tokens
```
...
~~MODEL_1 / MODEL_2 / MODEL_3~~ — removed in Task 6.
...
**Core (3 packages):**
- `@clack/prompts@^0.8.0` — Interactive CLI prompts (Task 5: replaced inquirer)
...
|-------|----------|----------|--------|
| `install.sh` wizard questions outdated | ⚠️ Medium | `scripts/install.sh` | Deferred (Wizard Redesign Backlog) |
| `token-limits.json` still 840+ lines | ⚠️ Low | `.ai/token-limits.json` | Exists for reference, not used by installer |
| No real tests | ⚠️ Low | `package.json scripts.test` | Future |
| `install.ps1` missing (Windows gap) | 🟡 Low | backlog | Deferred |
...
| Architecture analysis | `/ctx` → `/sculptor` → `/arbiter` |
...
```
today == 2026-02-22 → 📊 Same day, continuing (check session usage)
today != 2026-02-22 → 🟢 New day! Fresh token limits since last push
```

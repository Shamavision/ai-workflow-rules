...
| **Complexity** | 🟡 Medium (5/10) ← reduced after Task 2 |
...
## 🔴 CRITICAL: Dual-Structure Architecture (Task 2 updated)
...
.ai/AI-ENFORCEMENT  →   npm-templates/.ai/AI-ENFORCEMENT.md
.ai/rules/core.md   →   npm-templates/.ai/rules/core.md
.ai/rules/product   →   npm-templates/.ai/rules/product.md
...
|------|---------|-------------|
| `ROADMAP.md` | Source of truth — Tasks 1-7 + status | Medium (gitignored) |
...
| `.claude/commands/ctx.md` | /ctx skill — project context analysis | Light |
| `.claude/commands/sculptor.md` | /sculptor skill — architectural analysis | Light |

**⛔ NEVER read whole file:** `.ai/token-limits.json` (840+ lines, ~15k tokens alone)
...
**Completed:**
- ✅ Task 1: /ctx + /sculptor skills + auto-monitoring hook pipeline
- ✅ Task 2: Remove Windsurf/Continue.dev → Claude Code + Cursor only (rule-files 10→6)

**Active / Next:**
| Task | Status | Description |
|------|--------|-------------|
| Task 3 | 🔴 Next | Fix pre-commit hook: speed (<10s) + LANG-CRITICAL false positives |
| Task 4 | 🟡 Planned | Reduce presets: 4→2 (minimal + ukraine-full) |
| Task 5 | 🟡 Planned | Replace inquirer@8 → @clack/prompts |
| Task 6 | 🟡 Planned | TOKEN_PRESETS deduplication (bin/cli.js ↔ JSON) |
| Task 7 | ⏳ Deferred | Session memory anchor (recalculate after Task 2-4) |
...
```
Context Layer  → Session window % (AI knows exactly: X / 200k)
Rate Layer     → Provider throttling (estimated: Normal / High load)
Billing Layer  → Financial (N/A for subscription, $X.XX for API billing)
```

**2 Architecture Models (Task 2: MODEL_2 removed):**
- `MODEL_1`: Hard Token Billing — API providers (Anthropic, Mistral, Google, DeepSeek)
- `MODEL_3`: Fair Use Dynamic — Claude Pro, Cursor (limits UNKNOWN by design)
...
**Core (3 packages):**
- `inquirer@^8.2.6` — Interactive CLI prompts ⚠️ Legacy (Task 5: migrate to @clack/prompts)
...
|-------|----------|----------|------|
| Pre-commit hook: 5-15 min per commit | 🔴 High | `scripts/pre-commit` | Task 3 |
| LANG-CRITICAL false positives (.ru in rule text) | 🔴 High | `scripts/pre-commit` | Task 3 |
| `token-limits.json` is 840+ lines | ⚠️ Medium | `.ai/token-limits.json` | Future |
| `bin/cli.js` TOKEN_PRESETS diverge from token-limits.json | ⚠️ Medium | `bin/cli.js:15-45` | Task 6 |
| `inquirer@^8` is legacy (v9+ changed API) | ⚠️ Medium | `package.json` | Task 5 |
| No real tests | ⚠️ Medium | `package.json scripts.test` | Future |
...
| Architecture analysis | `/sculptor` → `PROPOSALS.md` |

---

## ⚠️ Architectural Warnings

1. **Sync trap:** Changing rule-files without updating all 6 pairs = silent bug. ~~10~~ → 6 files now.
